...
def move():
    ...
    for x in range(n):
        for y in range(n):
            # 상어가 존재하는 경우
            if graph[x][y] != 0:
                direction = directions[graph[x][y] - 1]     # 현재 상어의 방향
                found = False   # 냄새가 존재하지 않는 곳 발견 여부
                    ...
                    if 0 <= nx < n and 0 <= ny < n:
                        if smell[nx][ny][1] == 0:   # 냄새가 존재하지 않는 곳이면
                            ...
                            if new_graph[nx][ny] == 0:
                                ...
                                new_graph[nx][ny] = min(graph[x][y], new_graph[nx][ny])
                            found = True
                            break
                if found:
                    continue
                ...
                for index in range(4):
                    ...
                    ny = y + dy[priorities[graph[x][y] - 1][direction - 1][index] - 1]
                    if 0 <= nx < n and 0 <= ny < n:
                        if smell[nx][ny][0] == graph[x][y]: # 자신의 냄새가 있는 곳이면
                            # 해당 상어의 방향 이동시키기
                            directions[graph[x][y] - 1] = priorities[graph[x][y] - 1][direction - 1][index]
                            # 상어 이동시키기
                            new_graph[nx][ny] = graph[x][y]
...
while True:
    ...
    new_graph = move()  # 모든 상어를 이동시키기
    graph = new_graph   # 맵 업데이트
    time += 1           # 시간 증가
